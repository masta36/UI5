		...
		onTheDetailPage: {
			...
			assertions: {
				...
				iShouldSeeTheRememberedObjectId: function() {
					return this.waitFor({
						success: function() {
							var sBindingPath = "/Orders('" + this.getContext().currentId + "')";
							this._waitForPageBindingPath(sBindingPath);
						}
					});
				},
					...
					return this.waitFor({
						...
						success: function(oList) {
							...
							return this.waitFor({
								...
								matchers: new PropertyStrictEquals({
									name: "text",
									value: "Order_Details (" + iNumberOfItems + ")"
								}),
								success: function() {
				...
				theShareTileButtonShouldContainTheRememberedObjectName: function() {
					return this.waitFor({
						...
						matchers: function(oButton) {
							var sObjectName = this.getContext().currentListItem.getBindingContext().getProperty("OrderID");
